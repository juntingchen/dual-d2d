% This script generates simulation results depicted in Fig. 2a and case A 
% Fig. 2b in in the paper
%
...
% D2D_quality = [1     1     0.7
%                1     1     0.7
%                0.7     0.7     1]; % <- Scaling factor of the D2D CSI quality
%                                % 1 - some quality, 0 - no CSI at all
D2D_quality = [1     1     1
               1     1     1
               1     1     1]; % <- Scaling factor of the D2D CSI quality
                               % 1 - some quality, 0 - no CSI at all
SNR_vec = [0:5:25 20];      % SNR
...
%       Sigma = ones(K) - (1 - sd) * ones(K) * min(D2D_quality(:));
        ...
        W4 = ((1 - Sbf1) * H2 * Omega * H2' + (Sbf1 * Phi + K / P) * eye(Nt)) ...
                \ H2;
        [rsum(2, i), rates(:, 2, i)] = sum_rate_neql(H, W4, P);
    ...
    Rates_cell{i_case} = mean(rates, 3);
...
for i_case = 1:Ncase
    for k = 1:K
        for i_sch = 1:Nsch
            Rates(i_sch, i_case, k) = Rates_cell{i_case}(k, i_sch);
        end
    end
end
...
if Nsnr > 1
    ...
    xlabel('Total power (dB)');
    % title(sprintf('Nt = %d, K = %d, Sigma = %2.1f', Nt, K, sd_vec(end)));
    %ylim([0.5 3.5]);
...
if Nsd > 1
    ...
    for k = 1:K
        ...
        ylim([0 5]);
        set(gca, 'YTick', 0:5);
        figureFileName = sprintf('rate_ue%d_caseA', k);
